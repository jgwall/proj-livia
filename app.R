...
sidebar = dashboardSidebar(
  ...
  box(title="Population parameters",
      width="100%",
      status="info",
      solidHeader=TRUE,
      collapsible=TRUE,
  ...
  # Box for mutation parameters
  box(title="Mutation parameters",
      width="100%",
      status="info",
      solidHeader=TRUE,
      collapsible=TRUE,
...
body =  dashboardBody(
  ...
  fluidRow(
    box( title="Target Image",
         status="primary",
         solidHeader=TRUE,
         plotOutput("target_image")),
    box( title="Current Pop Average",
         status="success",
         solidHeader=TRUE,
         plotOutput("current_pop"))
  ),
  box(title="Population Fitness",
      status="warning",
      solidHeader=TRUE,
      width="100%",
...
server <- function(input, output, session) {
   ...
   image_resized = reactive({
     if(is.null(image_loaded()) ){ # Pass NULL if image not loaded yet
       return(NULL)
     }
   ...
   image_target = reactive({
     if(is.null(image_resized())){  # Pass NULL if image not loaded yet
       return(NULL)
     }
     if(input$grayscale){
